...
module "contributor_insights_enable" {
  for_each = var.contributor_insights_template_list

  source  = "digitickets/cli/aws"
  version = "5.0.4"

  aws_cli_commands = ["cloudwatch", "put-managed-insight-rules", "--managed-rules TemplateName=${each.key},ResourceARN=${var.service_arn}"]
}
